...
<div class="main">
  <div class="bg-white p-4">
    ...
    <div class="breadcrumb-stitle">
      ...
      <span class="txt">新增分类</span>
    </div>
  </div>

  <ul class="form-message m-4 p-4"></ul>
  <form class="mt-4" id="form" action="/admin/article/category_store" method="post" autocomplete="off">
    @csrf
      ...
      <div class="row mb-3">
        ...
        <div class="col-auto">
          <select class="form-select" name="parent_id">
            ...
            <option value="{{$value->id}}" @if(Request()->parent_id == $value->id) selected @endif>@php for($i = 1; $i < $value->level; $i++) {echo '&nbsp;&nbsp;';} @endphp {{$value->name}}</option>
            @endforeach
          </select>
        </div>
      </div>
    ...
    <div class="m-4 pagebox">
      ...
      <div class="mt-4">
        <div class="row mb-3">
          ...
          <div class="col-8">
            <input class="form-control" name="tpl_list" value="{{Config('common.view.article.tpl_list')}}">
          </div>
        </div>
      </div>
      <div class="mt-4">
        <div class="row mb-3">
          ...
          <div class="col-8">
            <input class="form-control" name="tpl_show" value="{{Config('common.view.article.tpl_show')}}">
          </div>
        </div>
      </div>
    </div>
