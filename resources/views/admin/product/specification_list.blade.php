...
<div class="main">
  <div class="bg-white p-4">
    <div class="breadcrumb-stitle">商品规格</div>
    @if(!empty($group))
    <div class="mt-3" style="font-size: 12px;">规格组合：{{$group->name}}</div>
    @endif
  </div>
  <div class="m-4">
    <div class="pagebox">
  ...
  <div class="mx-4">
    <div class="pagebox">
      <div class="overflow-hidden">
        <div class="float-start row g-2">
          <div class="col-auto" style="font-size: 12px; color: #999;">
            共有 {{$specifications->total()}} 条数据
          </div>
        </div>
        <div class="float-end row g-2">
          <div class="col-auto">
            <a class="btn btn-primary" href="/admin/product/specification_create?group_id={{Request()->group_id}}"><i class="iconfont luck-icon-jia"></i> 新增</a>
          </div>
          <div class="col-auto">
      ...
      <table class="table text-center mt-3">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>{{$value->name}}</td>
            <td>
              <div class="options" onclick="layerOpen('/admin/product/specification_option_list?specification_id={{$value->id}}', '选项值');">
            ...
            <td>
              <button type="button" class="btn btn-success btn-sm" onclick="layerOpen('/admin/product/specification_option_list?specification_id={{$value->id}}', '选项值');">选项值</button>
              <a class="btn btn-primary btn-sm" href="/admin/product/specification_edit?id={{$value->id}}{{ encodePrevPageParams(); }}">编辑</a>
      ...
      <div class="page my-3">{{ $specifications->appends(Request()->all())->render() }}</div>
      @else
      <div class="noresult">
...
function deleteAction(id = '') {
  ...
  var layer_confirm = layer.confirm('确认删除？', function(index) {
    ...
    $.ajax({
      type: 'GET',
      url: '/admin/product/specification_delete?id=' + id,
      dataType: 'json',
