...
<div class="main">
  ...
  <form class="mt-4" id="form" action="/admin/product/specification_store" method="post" autocomplete="off">
    @csrf
      ...
      <div class="row mb-3">
        ...
        <div class="col-auto">
          <select class="form-select" name="group_id">
            ...
            <option value="{{$value->id}}" @if($value->id == Request()->group_id) selected @endif>{{$value->name}}</option>
            @endforeach
          </select>
        </div>
      </div>
  ...
  submitHandler: function() {
    layer.confirm('确认提交？', function() {
      ...
      $("#form").ajaxSubmit(function(res) {
        layer.close(load);
        if (res.code == 200) {
          layer.msg('操作成功', { time: 1500 }, function () { window.location.href = '/admin/product/specification_list?group_id={{Request()->group_id}}'; });
