...
<div class="main">
  <div class="bg-white p-4">
    <ol class="breadcrumb">
      <li class="breadcrumb-item">商品规格组合</li>
  ...
  <form class="mt-4" id="form" action="/admin/product/specification_group_update" method="post" autocomplete="off">
    @csrf
    ...
    <div class="m-4 pagebox">
      <div class="row mb-3">
        ...
        <div class="col-8">
          <input class="form-control" name="name" value="{{$group->name}}" placeholder="">
        </div>
      </div>
      <div class="row mb-3">
      ...
      <div class="row mb-3">
        ...
        <div class="col-auto">
          <input class="form-control" name="sort" value="{{$group->sort}}" placeholder="">
        </div>
      </div>
      <div class="row mb-3">
        ...
        <div class="col-auto">
          <select class="form-select" name="status">
            ...
            <option value="{{$key}}" @if($group->status == "$key") selected @endif>{{$value}}</option>
            @endforeach
          </select>
        </div>
      </div>
  ...
  submitHandler: function() {
    layer.confirm('确认提交？', function() {
      ...
      $("#form").ajaxSubmit(function(res) {
        layer.close(load);
        if (res.code == 200) {
          layer.msg('操作成功', { time: 1500 }, function () { window.location.href = '/admin/product/specification_group_list{!! decodePrevPageParams(); !!}'; });
