...
<div class="main">
  <div class="bg-white p-4">
    <ol class="breadcrumb">
      <li class="breadcrumb-item">商品规格</li>
  ...
  <form class="mt-4" id="form" action="/admin/product/specification_update" method="post" autocomplete="off">
    @csrf
    ...
    <div class="m-4 pagebox">
      <div class="row mb-3">
        ...
        <div class="col-8">
          <input class="form-control" name="name" value="{{$specification->name}}" placeholder="">
        </div>
      </div>
      <div class="row mb-3">
        ...
        <div class="col-auto">
          <select class="form-select" name="group_id">
            ...
            <option value="{{$value->id}}" @if($value->id == $specification->group_id) selected @endif>{{$value->name}}</option>
            @endforeach
          </select>
        </div>
      </div>
      ...
      <div class="row mb-3">
        ...
        <div class="col-auto">
          <input class="form-control" name="sort" value="{{$specification->sort}}" placeholder="">
        </div>
      </div>
      <div class="row mb-3">
        ...
        <div class="col-auto">
          <select class="form-select" name="status">
            <option value="1" @if($specification->status == 1) selected @endif>开启</option>
            <option value="0" @if($specification->status == 0) selected @endif>关闭</option>
          </select>
        </div>
      </div>
    </div>
  ...
  submitHandler: function() {
    layer.confirm('确认提交？', function() {
      ...
      $("#form").ajaxSubmit(function(res) {
        layer.close(load);
        if (res.code == 200) {
          layer.msg('操作成功', { time: 1500 }, function () { window.location.href = '/admin/product/specification_list{!! decodePrevPageParams(); !!}'; });
...
function addOption() {
  ...
  str += '<div class="item">';
  str += option;
  str += '<input type="hidden" name="options_id[]" value="">';
