...
<div class="main">
  ...
  <form class="mt-4" id="form" action="/admin/product/specification_option_store" method="post" autocomplete="off">
    @csrf
    <div class="m-4 pagebox">
      <div class="row mb-3">
        <label class="col-2 col-form-label text-end"><span class="text-danger">*</span> 选项值：</label>
        <div class="col-8">
          <input class="form-control" name="option" placeholder="">
        </div>
      </div>
      <div class="row mb-3">
        <label class="col-2 col-form-label text-end">所属规格：</label>
        <div class="col-auto">
          <input type="text" readonly="" class="form-control-plaintext" value="{{$specification->name}}" />
  ...
  submitHandler: function() {
    layer.confirm('确认提交？', function() {
      ...
      $("#form").ajaxSubmit(function(res) {
        layer.close(load);
        if (res.code == 200) {
          layer.msg('操作成功', { time: 1500 }, function () { window.location.href = '/admin/product/specification_option_list?specification_id={{Request()->specification_id}}'; });
